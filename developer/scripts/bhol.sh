...
git push -u origin mai#!/bin/bash
...
if [[ -z "${MCW_GITHUB_URL}" ]]; then
    MCW_GITHUB_URL=https://$MCW_GITHUB_USERNAME:$MCW_GITHUB_TOKEN@github.com/$MCW_GITHUB_USERNAME/Fabmedical2.git
fi

git config --global user.email "$MCW_GITHUB_EMAIL"
git config --global user.name "$MCW_GITHUB_USERNAME"

cp -R ~/MCW-Cloud-native-applications/Hands-on\ lab/lab-files/developer ~/Fabmedical2
cd ~/Fabmedical2
git init
git remote add origin $MCW_GITHUB_URL
...
# Configuring github workflows
cd ~/Fabmedical2
sed -i "s/\[SUFFIX\]/$MCW_SUFFIX/g" ~/Fabmedical2/.github/workflows/content-init.yml
sed -i "s/\[SUFFIX\]/$MCW_SUFFIX/g" ~/Fabmedical2/.github/workflows/content-api.yml
sed -i "s/\[SUFFIX\]/$MCW_SUFFIX/g" ~/Fabmedical2/.github/workflows/content-web.yml
